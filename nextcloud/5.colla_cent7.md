# Cài đặt Nextcloud 20 trên Centos 7 + Collabora cùng 1 server

Sau khi cài đặt Nextcloud. Cài đặt Collabora theo các bước sau
...
Thông thường tích hợp collabora sẽ cần có SSL và domain.  

Nếu không có domain và SSL thì thực hiện bước sau. Cơ bản sẽ set enable SSL = false và termination ssl = false 
...
Với 2db21f32ef1c trong lệnh trên là docker id của mình. 

Dùng lệnh `docker ps` để biết docker id của mình là gì.

Đợi vài giây cho container tự restart sau đó ta kiểm tra bằng lệnh

    curl http://127.0.0.1:9980

Kết quả trả lại `OK` nghĩa là đã thành công.

## Cài đặt nginx làm reverse proxy cho collobora

    yum install nginx -y 

Cài chung server với nextcloud nên cần sửa port của nginx để tránh xung đột với dịch vụ httpd

    vi /etc/nginx/nginx.conf

Sửa dòng thứ 39 và 40 từ 
...
Kiểm tra lại ta có cấu hình sai ở đâu không

    nginx -t 

Kết quả trả lại như sau là thành công
...
Đăng nhập vào nextcloud với tài khoản nc_admin

Vào mục Setting -> Collabora Online Development -> Use your own server -> Nhập IP:8081 -> Click Save

![Imgur](https://i.imgur.com/PPtHPlL.png)

Upload thử các file office lên để kiểm tra hoạt động của Collabora. Kết quả sẽ như sau:

![Imgur](https://i.imgur.com/yucRr0Q.png)

![Imgur](https://i.imgur.com/JjJZ3BY.png)